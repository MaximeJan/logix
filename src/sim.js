...
// Importée à la fois par src/CircuitSimulator.jsx (rendu) et par
// tests/sim-core.mjs (tests Node.js). Permet de garantir que les deux
// utilisent strictement la même logique.
//
// La fonction `simulate()` reçoit `getDef` en argument : c'est ce qui rend
// ce module agnostique au format de définition (avec ou sans `shape` JSX).
// ============================================================
...
export function maskTo(width, v) {
  if (width >= 32) return v | 0;
  return (v | 0) & ((1 << width) - 1);
}

export const portKey = (compId, portName) => `${compId}:${portName}`;

// --------- Table de décodage 7 segments (hex) ---------
// bit 0 = a, bit 1 = b, ..., bit 6 = g
export const SEG7_HEX_TABLE = [
...
// Orientation 'right' = repère natif (sortie à droite). 'down' = rotation horaire 90°,
// 'left' = 180°, 'up' = 270°.
export function applyOrientation(def, orientation) {
...
// Tri topologique de Kahn sur le graphe des fils, évaluation des composants
// dans l'ordre. Toutes les valeurs sont des entiers (les buses sont des Number
// avec masquage à `width` bits).
// `getDef(type, customDefs, comp)` doit renvoyer la définition d'un type,
// incluant `inputs`, `outputs`, et éventuellement `fn` ou `isCustom`+`customCircuit`.
export function simulate(circuit, getDef, customDefs = null, recursionStack = new Set()) {
  const defs = customDefs ?? circuit.customDefinitions ?? {};
  const { components, wires } = circuit;
  const compMap = new Map(components.map((c) => [c.id, c]));
  // Fils valides uniquement : ceux qui pointent vers des composants existants des deux côtés.
  // Sans ce filtre, un fil orphelin (vers un composant supprimé) écraserait dans
  // wireToInput le bon fil qui arrive sur le même port.
  ...
  const incoming = new Map(components.map((c) => [c.id, new Set()]));
  for (const w of validWires) {
  ...
  for (const id of order) {
    ...
    if (comp.type === 'INPUT') {
      ...
      // Puits visuel : pas de sortie. L'écriture (front montant + WE)
      // est gérée hors-simulate. Le RST asynchrone met tout à zéro et
      // s'applique aussi lors de l'évaluation visuelle (cf stepSequential).
      outVals = [];
      ...
      if (recursionStack.has(comp.type)) {
        ...
        const childCircuit = { components: childComponents, wires: def.customCircuit.wires };
        ...
        outVals = def.outputs.map((p) =>
          asInt(childResult.inputValues.get(portKey(p.internalId, 'in0')) ?? 0)
        );
      }
    } else if (def.fn) {
...
// --------- Étape séquentielle ---------
// Applique la mise à jour des composants à mémoire (DFF, REG, COUNTER, RAM, SRLATCH)
// en fonction d'une simulation préalable. Renvoie un nouveau circuit avec les
// états mis à jour. Garantit l'atomicité (toutes les mises à jour se font à partir
// de `sim` calculée AVANT cette étape).
export function stepSequential(circuit, getDef) {
  ...
  const newComponents = circuit.components.map((comp) => {
    ...
    if (comp.type === 'LEDMATRIX') {
      ...
      if (rstVal) {
        ...
        if (allZero) {
          for (let i = 0; i < total; i++) if (asInt(pixels[i]) !== 0) { allZero = false; break; }
        }
        if (!allZero) {
        ...
        if (x >= 0 && x < cols && y >= 0 && y < rows) {
          ...
          if (asInt(pixels[idx] ?? 0) !== d) {
            ...
            // Normalise la taille au cas où mem a été redimensionnée à part
